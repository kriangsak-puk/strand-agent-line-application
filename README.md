...
Send it a message on LINE and it answers like a normal assistant, searching the web (via DuckDuckGo, no API key needed) whenever a question needs current or factual information it isn't sure about. The same agent code runs two ways: **entirely on your machine**, or **entirely in AWS** (Bedrock AgentCore Runtime + Lambda). Pick whichever fits, see the "How to run" section below.
